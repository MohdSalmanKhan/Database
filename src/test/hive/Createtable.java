...
public class Createtable 
{
	...
	static String printValue="Querytype:create\n";
	static String configValue="";

	
	public static void CreatetableMethod(String query,String configpath)
     {
    	

		configValue = configpath;
		flag=false;
/*		query="create table Idaentificatioan_newwwd (\n" +
                "  name varchar(50),\n" +
                "  street varchar(50),\n" +
                "  city varchar(50),\n" +
                "  state varchar(50),\n" +
...
*/
    ...
    {
    	int i=0;
    	String writeIntoConfig="BEGIN\n";
    	String writeIntoFileData="";
    	String table = pStmt.getTargetTable().toString().trim();
    	BufferedWriter bw;
    	FileWriter fw;
        //System.out.println("Table Name:"+pStmt.getTargetTable().toString());
      //  String tableNames[] = db.rf.tableName;
    	...
    	while(itr.hasNext())
        {
           ...
           // System.out.println("Table Name:"+key);
            if((table+".csv").compareToIgnoreCase(key)==0)
            {	
  ...
  //      System.out.println("flag="+flag);
        if(flag==false)
        {	printValue = printValue +"Tablename:"+ table+"\nAttributes:";
        	writeIntoConfig = writeIntoConfig + table + "\n";
	        for(i=0;i<pStmt.getColumnList().size();i++)
	        {
	            column = pStmt.getColumnList().getColumn(i);
	            String token[] = column.toString().split(" ");
	//            System.out.println("col="+token[0]);
	            printValue = printValue + token[0].trim()+" ";
	            writeIntoConfig = writeIntoConfig + token[0] +", ";
	            writeIntoFileData = writeIntoFileData + token[0] +":";
	          //  System.out.println("\tname:"+column.getColumnName().toString());
	            String dataType = column.getDatatype().toString();
	            printValue = printValue + dataType.trim() + ",";
	  //          System.out.println("datatype="+dataType);
	            writeIntoConfig = writeIntoConfig + dataType + "\n";
	            writeIntoFileData = writeIntoFileData + dataType + ",";
	            int index1 = dataType.indexOf("(");
	            int index2 = dataType.indexOf(")");
	            String varcharChecker="";
	            if(index1!=-1 && index2!=-1)
	            {
	        //    	System.out.println("index1="+index1+"\tindex2="+index2);
	            	String size = dataType.substring(index1+1, index2);
	            	int value;
	            	try
	            	{
	            	...
	            	varcharChecker=dataType.substring(0,index1);
	            }
	            if(dataType.compareToIgnoreCase("integer") == 0 || dataType.compareToIgnoreCase("float") == 0 || varcharChecker.compareToIgnoreCase("varchar")==0 )
	            {
      ...
      /*      if (column.getDefaultExpression() != null)
        ...
        {
        	writeIntoConfig = writeIntoConfig + "END\n";
        	writeIntoFileData = writeIntoFileData.substring(0, writeIntoFileData.length()-1);
       
        	try
        	{
        		//for csv
        		fw = new FileWriter(db.rf.pathForData+table+".csv");
        		bw =new BufferedWriter(fw);
     //   		fw.close();
        		bw.close();
        		
        		// for data file
        		fw = new FileWriter(db.rf.pathForData+table+".data");
        		bw =new BufferedWriter(fw);
        		bw.write(writeIntoFileData);
      //  		fw.close();
        		bw.close();
        		
        		// for config file
        		fw = new FileWriter(configValue,true);
        		bw =new BufferedWriter(fw);
        		bw.write(writeIntoConfig);
       // 		fw.close();
        		bw.close();
